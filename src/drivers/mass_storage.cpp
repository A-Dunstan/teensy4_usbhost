...
void USB_Storage::reset(void) {
  ...
  // bulk storage reset
  ControlMessage(bmrtReset, USBMS_REQ_RESET, 0, interface);
  delay(100);
  // clear HALT on IN endpoint
  ControlMessage(USB_REQTYPE_ENDPOINT_SET, USB_REQ_CLEAR_FEATURE, USB_FEATURE_ENDPOINT_HALT, bulk_in);
  delay(100);
  // clear HALT on OUT endpoint
  ControlMessage(USB_REQTYPE_ENDPOINT_SET, USB_REQ_CLEAR_FEATURE, USB_FEATURE_ENDPOINT_HALT, bulk_out);
  delay(100);
}
...
int USB_Storage::scsi_cmd(uint8_t lun, void* data, size_t length, const uint8_t* command, bool write) {
  ...
  while (transferred < length) {
    ...
    if (ret < (int)to_send) {
      dprintf("scsi_cmd: short data transfer %d vs %lu\n", ret, to_send);
  ...
  if (xfer.csw.dCSWSignature == CSW_SIGNATURE && xfer.csw.dCSWTag == t) {
    if (xfer.csw.bCSWStatus == 0) { // success
      ...
      if (residue != (length - transferred)) {
        dprintf("SCSI: Residue mismatch, expected %u but received %lu\n", length - transferred, residue);
      }
      else if (residue)
        dprintf("SCSI RESIDUE: %lu\n", residue);
